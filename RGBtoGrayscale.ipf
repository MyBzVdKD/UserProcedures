...
function /Wave RGB2Gray_wColor(SWN_rgbMatrix, V_Color2GrayLev, F_kill)
	String SWN_rgbMatrix
	variable V_Color2GrayLev, F_kill
	Prompt SWN_rgbMatrix, "rgb matrix:",popup,WaveList("!*Gray",";","")
	wave W_rgbMatrix=$SWN_rgbMatrix
	String SWN_luminance= SWN_rgbMatrix+"Gray"
	...
	// Coefficients from "Graphics Gems", RGB to Y (I and Q discarded)
	wave W_luminance=$SWN_luminance
	multithread W_luminance =W_rgbMatrix[p][q][0]*(W_rgbMatrix[p][q][0]==W_rgbMatrix[p][q][1])*(W_rgbMatrix[p][q][1]==W_rgbMatrix[p][q][2])
	multithread W_luminance +=V_Color2GrayLev*((W_rgbMatrix[p][q][0]!=W_rgbMatrix[p][q][1])+(W_rgbMatrix[p][q][1]!=W_rgbMatrix[p][q][2])>=1)
	//0.299*W_rgbMatrix[p][q][0] + 0.587*W_rgbMatrix[p][q][1] + 0.114*W_rgbMatrix[p][q][2]
	if(F_kill)
		killwaves W_rgbMatrix
	endif
	//Print "output is "+luminance
