...
function /wave List2wave(L_src, SWN_dest, S_listSepStr)
	string L_src, SWN_dest, S_listSepStr
	variable V_n=ItemsInList(L_src  , S_listSepStr )
	make /O/n=(V_n), $SWN_dest
	wave W_dest=$SWN_dest
	//variable i
	//for(i=0;i<V_n;i+=1)
		W_dest= str2num(StringFromList(p, L_src , S_listSepStr))
//	endfor
	return W_dest
end


function /s WaveSeparation(W_src, LWN_dest, S_listSepStr, V_stat, V_end, S_unit)
	wave W_src
	string LWN_dest, S_listSepStr
	variable V_stat, V_end
	string S_unit
	variable V_n=ItemsInList(LWN_dest  , S_listSepStr )
	variable V_npnts=numpnts(W_src)/V_n
	string SWN_dest
	variable i
	for(i=0;i<V_n; i+=1)
		SWN_dest= StringFromList(i, LWN_dest, S_listSepStr )
		//print SWN_dest		
		make /O/n=(V_npnts) $SWN_dest
		wave W_dest=$SWN_dest
		SetScale/I x V_stat, V_end,S_unit, W_dest
		multithread W_dest=W_src[i+V_n*p]
	endfor
	return LWN_dest
end

unction /S dec2bin(V_Dval)
	variable V_Dval
    //variable hex_value
    string S_dest
	sprintf S_dest, "%b" V_Dval
	//variable V_ strlen(S_dest)
	return S_dest
end

function MovieMaker3D(M_3Dsrc, V_fps)
	wave M_3Dsrc
	variable V_fps
	variable V_n=dimsize(M_3Dsrc, 2 )
	newmovie /F=(V_fps) /CF=0 as nameofwave(M_3Dsrc)
	variable i
	string S_sprintf
	for(i=0;i<V_n;i+=1)
		ModifyImage $nameofwave(M_3Dsrc) plane=(i)
		sprintf S_sprintf, "\\Z24Position / %4.1f μm", IndexToScale(M_3Dsrc, i, 2)
		//TextBox/C/N=text1 "\\Z24Position / "+num2str(IndexToScale(M_3Dsrc, i, 2)) +" μm"
		
		TextBox/C/N=text1 S_sprintf
		AddMovieFrame
	endfor
	CloseMovie
end
